...
def generate_scalability_data():
    ...
    for i, event in enumerate(events, start=1):
        ...
        ll.append(event)
        if i % 100000 == 0:
            print(f"Loaded {i}/{n} events", flush=True)
...
if __name__ == "__main__":
    print("Starting scalability challenge at", time.strftime('%Y-%m-%d %H:%M:%S'))
    print(" Generating one million random events ...")
    arr, ll, gen_time = generate_scalability_data()
    print(f"Event generation complete at {time.strftime('%Y-%m-%d %H:%M:%S')}.")
    print("Attempting Merge Sort on array ...")
    merge_array = merge_sort_large_array(arr)
    print(f"Merge sort for array complete at {time.strftime('%Y-%m-%d %H:%M:%S')}.")
    print("Attempting Merge Sort on linked list ...")
    merge_ll = merge_sort_large_linked_list(ll)
    print(f"Merge sort for linked list complete at {time.strftime('%Y-%m-%d %H:%M:%S')}.")
    ...
    bin_ll = binary_search_large_linked_list(target_id, ll)
    print(f"Binary Search Sort on linked list complete at {time.strftime('%Y-%m-%d %H:%M:%S')}.")
    print("Attempting Binary Search Sort on array ...")
    bin_array = binary_search_large_array(target_id, arr)
    print(f"Binary Search Sort on array complete at {time.strftime('%Y-%m-%d %H:%M:%S')}.")
